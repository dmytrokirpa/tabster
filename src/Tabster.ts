...
/**
 * Creates a new isolated Tabster instance without using the window singleton.
 * Each call returns a fresh, independent context — useful for testing, SSR,
 * or iframe isolation. Features are still opt-in via getGroupper(), getMover(), etc.
 */
export function createIsolatedTabster(
    win: Window,
    props?: Types.TabsterCoreProps
): Types.Tabster {
    const core = new TabsterCore(win, props);
    return core.createTabster();
}
